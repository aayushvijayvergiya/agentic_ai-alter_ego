...
class TestNotificationService:
    ...
    @pytest.mark.anyio
    @patch('smtplib.SMTP')
    async def test_send_notification_smtp_success(self, mock_smtp):
        """Test successful SMTP notification sending."""
        # Setup mock
        ...
        with patch.dict('os.environ', env_vars):
            from src.alter_ego.config import Config
            with patch('src.alter_ego.services.notification_service.config', Config()):
    ...
    async def test_send_notification_pushover_success(self, mock_post):
    ...
    async def test_send_notification_no_config(self):
        """Test notification when no notification channel is configured."""
        with patch.dict('os.environ', {}, clear=True):
            from src.alter_ego.config import Config
            with patch('src.alter_ego.services.notification_service.config', Config()):
                service = NotificationService()
...
class TestGitHubService:
    ...
    async def test_fetch_profile_and_repos(self, mock_get: Mock, mock_cache: Mock, mock_cached: Mock) -> None:
        ...
        async def side_effect(url: str, **kwargs: object) -> Mock:
        ...
        content = await service.get_github_content()
    ...
    @patch("src.alter_ego.services.github_service.GitHubService._cache_content")
    @patch("httpx.AsyncClient.get")
    async def test_graceful_fallback_on_api_error(self, mock_get: Mock, mock_cache: Mock, mock_cached: Mock) -> None:
    ...
    async def test_cache_is_used_when_fresh(self, mock_exists: Mock) -> None:
    ...
    @pytest.mark.anyio
    @patch("httpx.AsyncClient.get")
    async def test_forks_excluded_from_repos(self, mock_get: Mock) -> None:
        ...
        mock_client.get.return_value = self._make_mock_response(repos)

        service = GitHubService()
        owned = await service._get_top_repos(mock_client, "aayushvijayvergiya")
